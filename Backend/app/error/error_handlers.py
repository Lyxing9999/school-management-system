...
def register_error_handlers(app):
    ...
    def handle_custom_validation(error: PydanticBaseValidationError):
        return Response._prepare_error_response(error=error), getattr(error, "status_code", 400)
    ...
    def handle_not_found(error: NotFoundError):
        return Response._prepare_error_response(error=error), getattr(error, "status_code", 404)

    @app.errorhandler(DatabaseError)
    def handle_database_error(error: DatabaseError):
        return Response._prepare_error_response(error=error), getattr(error, "status_code", 500)
    
    @app.errorhandler(AuthenticationError)
    def handle_authentication_error(error: AuthenticationError):
        return Response._prepare_error_response(error=error), getattr(error, "status_code", 401)
    ...
    def handle_forbidden(error: ForbiddenError):
        return Response._prepare_error_response(error=error), getattr(error, "status_code", 403)
