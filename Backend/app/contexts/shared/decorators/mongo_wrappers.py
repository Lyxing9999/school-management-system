...
def mongo_operation(operation_name: str):
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            mixin = self_obj if isinstance(self_obj, MongoErrorMixin) else MongoErrorMixin()

            module_name = self_obj.__class__.__name__ if self_obj is not None else func.__module__
            logger = getattr(self_obj, "logger", None) or LogService.get_instance()
            ...
            try:
                ...
                logger.log(
                    ...
                    level="ERROR",
                    module=module_name,
                    extra={"operation": operation_name, "error": str(e), "success": False},
                )

                mixin._handle_mongo_error(operation_name, e)
