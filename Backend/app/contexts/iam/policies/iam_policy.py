...
class IAMPolicy:
    ...
    def _get_role_active(self, user_id: ObjectId) -> str | None:
        user = self.iam.find_one(not_deleted({"_id": user_id}), {"role": 1})
        if not user:
            return None
        return user.get("role")

    def _get_role_any(self, user_id: ObjectId) -> str | None:
        user = self.iam.find_one(by_show_deleted("all", {"_id": user_id}), {"role": 1})
        if not user:
            return None
        return user.get("role")

    def can_soft_delete_user(self, user_id: ObjectId) -> PolicyResult:
        role = self._get_role_active(user_id)
        if not role:
    ...
    def can_restore_user(self, user_id: ObjectId) -> PolicyResult:
        role = self._get_role_any(user_id)
        if not role:
