...
async function finalizeRun(row: PayrollRunDTO) {
  try {
    await ElMessageBox.confirm(
      `Finalize payroll run ${displayRelation(
        row.payroll_run_label,
        row.payroll_month || row.month,
      )}?`,
...
async function markRunPaid(row: PayrollRunDTO) {
  try {
    await ElMessageBox.confirm(
      `Mark payroll run ${displayRelation(
        row.payroll_run_label,
        row.payroll_month || row.month,
      )} as paid?`,
    ...
    <div v-loading="detailLoading">
      <ElDescriptions v-if="activeRun" :column="1" border>
        ...
        <ElDescriptionsItem label="Generated By">
          {{
            displayRelation(activeRun.generated_by_name, activeRun.generated_by)
          }}
        </ElDescriptionsItem>
